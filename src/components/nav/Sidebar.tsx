...
export default function Sidebar() {
  ...
  return (
    <>
      {/* ── Desktop sidebar ── */}
      <aside className="hidden md:flex h-full w-60 flex-col border-r border-stone-200 bg-white">
        {/* Brand */}
            ...
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <li key={href}>
                  ...
                  >
                    <Icon
                      className={`h-4 w-4 flex-shrink-0 ${active ? 'text-brand-600' : 'text-stone-400'}`}
                    />
                    {label}
                  </Link>
                </li>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 flex border-t border-stone-200 bg-white">
        {bottomNavItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon className={`h-5 w-5 ${active ? 'text-brand-600' : 'text-stone-400'}`} />
